...
in
{
  options.programs.noctalia.settings = lib.mkOption {
    type = tomlFormat.type;
    default = { };
    description = "Noctalia settings. Converted to TOML and placed at ~/.config/noctalia/config.toml";
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.user.tmpfiles.rules = [
      "L+ %h/.config/noctalia/config.toml - - - - ${tomlFormat.generate "noctalia-config.toml" cfg.settings}"
    ];
  };
}
